...
任务目标
- {pool_goal}
- 输出极简打新优先级：用飞书友好的窄卡片列表展示综合评分、融资倍数/认购热度、绿鞋/基石、申购冲突、首日回测映射。
...
- 必须按当前 IPO 池内标的的申购截止日、配发结果公告日/退款日判断“申购冲突”：只比较本次报告中的仍可认购标的，以及 `/hkipo --all` 纳入且会影响资金占用的已截止待配发标的；不要和历史旧批次或回测样本对比。同一申购截止日，或 B 的申购截止日在 A 的配发/退款日前，标为“同批次资金冲突”；若 B 的申购截止日在 A 的配发/退款日后，标为“可先申购 A，等 A 结果/退款后再申 B”。配发结果日缺失时，用上市日前一港股交易日作近似并标注“估算”。必须列出冲突对象和绝对日期；若用户点名 A 是否与 B/C 冲突，要直接回答能否资金先集中申购 A，等 A 结果/退款后再申 B/C。
- 回测校准只保留关键映射结论，写入每只 IPO 的“回测”字段；不要单列推导过程。
- 不输出目标价、确定性承诺或买卖指令；“推荐”只表达为打新/跟踪优先级。
- 不要使用 Markdown 大标题 `#` / `##`；飞书里标题会过大且上下间距不稳定。使用普通加粗标签和短分隔。
- 不要使用宽 Markdown 表格；飞书卡片会横向滚动且裁切。每只 IPO 用 6 行窄卡片字段展示。
- 每个加粗小节标题和每条 emoji 字段上方都保留一个空行，避免飞书卡片把小点挤在一起；尤其是 `💡 关键结论`、`📌 优先级`、`⏱ 申购冲突`。
