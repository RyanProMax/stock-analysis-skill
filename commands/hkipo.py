...
任务目标
- {pool_goal}
- 输出极简打新优先级：用报告正文里的窄字段列表展示综合评分、融资倍数/认购热度、绿鞋/基石、申购冲突、首日回测映射。
...
- 报告正文不要使用宽 Markdown 表格；每只 IPO 用 6 行窄字段块展示。
- 每个加粗小节标题和每条 emoji 字段上方都保留一个空行，避免报告正文小点挤在一起；尤其是 `💡 关键结论`、`📌 优先级`、`⏱ 申购冲突`。
