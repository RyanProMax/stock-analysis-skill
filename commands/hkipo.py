...
任务目标
- {pool_goal}
- 输出极简打新优先级：用顶层“申购冲突”小节展示资金时间线，用窄字段列表展示每只 IPO 的综合评分、融资倍数/认购热度、绿鞋/基石、首日回测映射。
...
- 必须按当前 IPO 池内标的的申购截止日、配发结果公告日/退款日判断“申购冲突”：只比较本次报告中的仍可认购标的，以及 `/hkipo --all` 纳入且会影响资金占用的已截止待配发标的；不要和历史旧批次或回测样本对比。同一申购截止日，或 B 的申购截止日在 A 的配发/退款日前，标为“同批次资金冲突”；若 B 的申购截止日在 A 的配发/退款日后，标为“可先申购 A，等 A 结果/退款后再申 B”。配发结果日缺失时，用上市日前一港股交易日作近似并标注“估算”。必须列出冲突对象和绝对日期；若用户点名 A 是否与 B/C 冲突，要直接回答能否资金先集中申购 A，等 A 结果/退款后再申 B/C。申购冲突必须写成顶层 `**⏱ 申购冲突**` 小节，和 `💡 关键结论`、`📌 优先级` 同级；不要在每只 IPO 字段块内输出 `⏱ 申购冲突`。
...
- 只在顶层小节之间保留空行，尤其是 `💡 关键结论`、`⏱ 申购冲突`、`📌 优先级`、`🔗 来源`。个股字段行之间不要空行，保持 `📍 阶段`、`💰 热度`、`🛡 结构`、`📈 回测`、`⚠️ 风险` 连续紧凑。
- 用固定 emoji 强化重点：🟢 高优先级，🟡 重点跟踪，⚪ 观察；💰 热度，🛡 结构，⏱ 申购冲突，📈 回测，⚠️ 风险，🔗 来源。
...
- 当前 IPO 池内资金冲突/可先申购 A 后申购 B；列出对象、申购截止日、配发/退款日和判断。
