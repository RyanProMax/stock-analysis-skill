...
- 输出极简打新优先级：综合评分、融资倍数/认购热度、绿鞋/基石、首日回测映射全部整合进一张表。
...
- 回测校准只保留关键映射结论，写入表格“首日回测映射”列；不要单列推导过程。
- 不输出目标价、确定性承诺或买卖指令；“推荐”只表达为打新/跟踪优先级。
- 输出必须短：除 Sources 外，不写表格外的逐票解释；结论先行最多 3 条 bullet。
...
## Sources
- 只列关键链接；每个标的最多 2-3 个来源，标明用途、发布日期/更新时间、Futu 当前字段或外部补充字段。"""
