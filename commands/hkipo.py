...
- 热度字段必须按固定顺序核验：先用 Futu/OpenD 当前字段，再查同一 IPO 当日券商新股中心 / 孖展表，再查当日财经门户新股频道的公开认购倍数 / 一手中签率，最后才用暗盘或其他二级热度。每个孖展 / 公开认购 / 暗盘数值都必须标注来源更新时间；同一字段多个来源冲突时，用更新时间最新且不晚于 {today} 的数值，旧值只可写“过期/仅供趋势参考”，不得进入当前热度主评分。
