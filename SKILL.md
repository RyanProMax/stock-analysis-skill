...
仓库同时提供一个面向外部 Agent 的低 token 轮询脚本：

```bash
python scripts/poll_realtime_quotes.py --symbols 600000,510300,159915
```

这个脚本返回单个 JSON 对象，适合外部 Agent 做定时批量轮询；字段使用稳定的机器可读结构，不要求消费方解析自由文本。
...
1. 检查 Python 是否可用， 版本要求 3.9+
2. 检查 `tushare` 包是否已安装·
3. 检查当前工作目录或 skill 根目录下的 `.env`，以及系统环境中 `TUSHARE_TOKEN` 是否已配置；如需覆盖默认接口地址，再检查 `TUSHARE_HTTP_URL`
...
全部数据接口，请参考 `references/api_reference.md`。
