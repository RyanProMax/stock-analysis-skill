...
- 必须根据当前 IPO 池内标的的申购截止日、配发结果公告日/退款日输出顶层“⏱ 申购冲突”小节，和“💡 关键结论”“📌 优先级”同级；只比较本次报告中的可认购标的，以及 `/hkipo --all` 纳入且仍影响资金占用的已截止待配发标的；不要和历史旧批次或回测样本对比。小节需说明是否可先集中申购 A，等 A 结果/退款后再申 B/C，并列出对象和绝对日期。
- 默认输出极简报告：普通加粗标题、结论最多 3 条、飞书友好的窄卡片列表、短链接 Sources；Futu 当前字段、外部补充热度、回测映射、发行结构、基本面/估值和风险整合进每只 IPO 的短字段；申购冲突只放在顶层小节，不在每只 IPO 卡片重复。不要使用 `#` / `##` Markdown 大标题或宽 Markdown 表格。只在 `💡 关键结论`、`⏱ 申购冲突`、`📌 优先级`、`🔗 来源` 等顶层小节前保留空行；`📍 阶段`、`💰 热度`、`🛡 结构`、`📈 回测`、`⚠️ 风险` 等个股小点之间不要额外空行。用少量固定 emoji 强化重点：🟢/🟡/⚪ 优先级，💰热度，🛡结构，⏱申购冲突，📈回测，⚠️风险，🔗来源。
