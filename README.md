...
`/hkipo` 必须按当前日期重新取数。默认只输出当前仍可认购的港股 IPO，过滤 Futu/OpenD 返回的 `is_subscribe_status=false` 已截止新股；用户显式发送 `/hkipo --all` 时才纳入已截止认购但未上市标的。当前 IPO 池、招股状态、上市日、招股截止日、发售价、一手股数和入场费优先使用 Futu/OpenD 只读 `get_ipo_list(HK)`；命令由 executor 按当前 skill 安装目录动态生成，不能依赖用户工作区相对 `.venv`。Futu/OpenD 不可用或字段缺失时，才用 HKEX / 公司公告 / 财经站补齐并标注降级。财经站只补充孖展/认购热度、中签率、灰市、首日涨幅等二级数据，且不得把过期数据当作当前热度评分依据。输出只约束最终报告正文：不用 `#` / `##` 大标题和宽 Markdown 表格；结论最多 3 条，核心内容全部整合进窄字段块列表，用少量固定 emoji 强化优先级、热度、结构、申购冲突、回测、风险和来源；每个加粗小节标题和每条 emoji 字段上方保留空行，避免小点挤在一起。申购冲突只比较当前报告 IPO 池里的资金时间线，按申购截止日、配发结果公告日/退款日判断同批次资金冲突，或是否可先集中申购 A、等 A 结果/退款后再申 B/C，并写入每只 IPO 字段块的 `⏱ 申购冲突` 字段；不要和历史旧批次或回测样本对比。评分、融资倍数热度、绿鞋/基石检查、回测映射和字段块输出规则见 [references/hkipo.md](./references/hkipo.md)。
