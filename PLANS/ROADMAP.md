...
- [x] `/research` 支持股票名输入，由 agent 识别唯一代码后再调用 CLI
...
- 2026-05-03：`/research` 支持股票名输入，executor 不做本地缓存或硬编码匹配；由 agent 先用可核验来源识别唯一市场和代码，再替换 CLI 模板调用 `stock_analyze.py`，多候选时先澄清。
