...
- [x] 固定 `/research` 标准 CLI 的 `uv` 解析，避免服务重启后 PATH 漂移
...
- 2026-05-04：`/research` 支持股票名输入，executor 不做本地缓存或硬编码匹配；原始输入传给上游 `stock-analysis-api` CLI 解析，返回 `identity_conflict` / `identity_not_found` 时先澄清。

- 2026-05-04：`/research` 研报模板新增行业整体趋势、市场热度、同类公司平均 PE 和权威机构研报汇总；要求说明数据日期、可比口径、机构观点共识与分歧，缺失时进入降级说明。
