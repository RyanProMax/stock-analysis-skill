...
- [x] `/research` 支持 A 股股票名自动匹配代码并生成标准 CLI
...
- 2026-05-03：`/research` 支持 A 股股票名输入，executor 读取本地 `stock-analysis-api` 标的缓存匹配代码，唯一匹配后生成标准 `stock_analyze.py` CLI，多候选或无候选时先澄清。
