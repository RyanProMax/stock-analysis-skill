...
> 本文件记录 `stock-analysis-skill` 对 Futu/OpenD 能力的统一路由、输出模板和安全边界。本仓库不复制富途脚本，不直接实现行情、账户或交易逻辑；Futu/OpenD 统一通过 `stock-analysis-api/scripts/futu_market_data.py` 或后续 API provider 扩展接入。尚未迁入 API 的能力必须明确标记为未支持，不再路由到外部 Futu skill。通过本 skill 使用 Futu/OpenD 时只允许查询操作。
...
| 港股 / 美股 / 多市场 snapshot、K 线、市场状态 | `stock-analysis-api` / `scripts/futu_market_data.py` | 当前支持 `global-state`、`snapshot`、`kline` |
| 盘口、逐笔、分时、期权链、窝轮 / 牛熊证、资金流、经纪队列、条件选股 | 未迁入 API | 明确返回暂不支持，并进入 API provider backlog |
| 账户、资金、持仓、订单、成交、流水 | 未迁入 API | 仅允许未来以只读 API provider 接入；当前不得绕路 |
...
| 标的包含 `HK.` / `US.` 等 API Futu CLI 已支持市场前缀，且只需 snapshot / K 线 / 市场状态 | `stock-analysis-api` / `scripts/futu_market_data.py` | 需要 Futu 多市场代码体系 |
| 用户明确说“富途”“OpenD”“牛牛”“Futu” | API Futu CLI 已支持能力 | 尊重用户指定数据源，但不绕回非 API provider |
| 需要盘口、逐笔、分时、期权、盘前盘后细粒度查询 | 未迁入 API | 明确返回暂不支持，不伪造结果 |
| 需要账户、持仓、订单、成交、流水查询 | 未迁入 API | 当前不支持；未来只能以只读 API provider 接入 |
